function fish_prompt
    # Store status for later since it would be overwritten by the commands in this file
    set -l prev_status $status

    # Add the time at the beginning of the prompt before it is being written to the history
    if contains -- --final-rendering $argv
        echo -n (set_color magenta)'['(date +%T)']'
    end

    # username and hostname for ssh prompt
    if test -n "$SSH_TTY"
    ...
    # If git is installed, add git information
    if command -sq git
        ...
        if set -l git_dir (command git rev-parse --git-dir 2>/dev/null)
            ...
            if test -n "$branch"
                ...
                set -l short_branch (echo $branch | \
                                    string replace -r '^bugfix/' 'b/' | \
                                    string replace -r '^feature/' 'f/' | \
                                    string replace -r '^release/' 'r/')
                echo -n "$short_branch"
            end
            if test -n "$commit"
    ...
    # Show status of previous command if not 0
    if test $prev_status -ne 0
        echo -n (set_color red)"[$prev_status]"
    end
